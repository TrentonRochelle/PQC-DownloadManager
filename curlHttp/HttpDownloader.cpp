...
size_t HTTPDownloader::writeData(void *ptr, size_t size, size_t nmemb, FILE *stream) {
...
void HTTPDownloader::download(std::ostream& output, std::string uri, std::string mimetype, size_t maxSize, long timeoutSeconds, std::string filePath)
{
    // char outfilename[FILENAME_MAX] = "/mnt/c/Users/Trenton Rochelle/Coding/Summer2019/PQC_DownloadManager/HttpDownloader-master/test.pdf";
    ...
    // Set up our custom handlers to deal with the data. TransferData
    ...
    curl_easy_setopt(_curl.get(), CURLOPT_WRITEDATA, fp);
